...
describe("dateHelpers", () => {
  ...
  it("adds days to ISO date strings while preserving local dates", () => {
    expect(addDaysISO("2025-12-01", 7)).toBe("2025-12-08");
    expect(addDaysISO("2025-12-01", -1)).toBe("2025-11-30");
  });

  it("normalizes arbitrary dates to their week's Monday", () => {
    expect(normalizeWeekOf("2025-12-04")).toBe("2025-12-01");
    expect(normalizeWeekOf("2026-01-07")).toBe("2026-01-05");
  });
  ...
  it("handles DST transitions and month boundaries without shifting dates", () => {
    expect(addDaysISO("2025-03-09", 1)).toBe("2025-03-10"); // spring forward
    expect(addDaysISO("2025-11-02", 1)).toBe("2025-11-03"); // fall back
  ...
  it("formats dates as YYYY-MM-DD at local midday to avoid timezone drift", () => {
    ...
    expect(toISODate(nearMidnight)).toBe("2025-12-31");
  });
});
